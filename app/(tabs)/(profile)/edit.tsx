...
export default function EditProfileScreen() {
  ...
  const userPictureApi = useApiPatch<UserResponse, FormData>(
    ApiRoutes.userPicture.userPicture(profile?.id || ''),
  );
  ...
  async function handleUpdate() {
    ...
    if (selectedImage && token) {
      const fd = buildAvatarFormData(selectedImage);
      ...
      try {
        ...
        console.warn('Upload OK:', res.data);
      } catch (err: any) {
    ...
    // const picturePromise = selectedImage
    //   ? userPictureApi.patch(buildAvatarFormData(selectedImage))
    //   : Promise.resolve({ success: true } as any);

    const [profileResponse, specialtyResponse] = await Promise.all([
      profilePromise,
      specialtyPromise,
    ]);

    if (!profileResponse?.success || !specialtyResponse?.success) {
      console.error(
        'Update error:',
        api.error ||
          userSpecialtyApi.error ||
          userPictureApi.error ||
          'Unknown error',
      );
                        ...
                        {fields.neighborhoods.value.map(
                          (neighborhood: string, idx: number) => (
                            ...
                            >
                              ...
                              {idx === fields.neighborhoods.value.length - 1 &&
