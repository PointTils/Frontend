...
export default function MultiSelect({
  ...
  const renderOption = ({ item }: { item: string }) => {
    ...
    return (
      ...
      >
        <Text
          className={`font-ifood-regular ${isSelected ? 'font-bold text-typography-white bg-primary-blue-light dark:bg-primary-blue-dark rounded' : ''}`}
        >
          {item}
        </Text>
      </TouchableOpacity>
  ...
  const renderSelectedTags = () => {
    if (selected.length === 0) {
      return (
        <Text className="font-ifood-regular w-fit flex-nowrap text-typography-400">
          {placeholder}
        </Text>
      );
      ...
      return (
        <View className="flex-row flex-wrap">
          ...
          <View className="bg-primary-blue-light dark:bg-primary-blue-dark px-2 rounded-full mr-2">
            <Text className="font-ifood-regular text-typography-white  text-sm">
              +{selected.length - 1}
            </Text>
          </View>
        </View>
  ...
  return (
    <View className={`relative ${width || 'w-full'}`}>
      ...
      >
        ...
        {open ? (
          <ChevronUp className="size-20 outline-primary-300" />
        ) : (
          <ChevronDown className="size-20 outline-primary-300" />
        )}
      </TouchableOpacity>
